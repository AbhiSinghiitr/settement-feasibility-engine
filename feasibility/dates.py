"""Calendar-date helpers: end-of-month clamping and monthly cadence generation.

Creditor payments and program fees recur monthly on their own cadence,
independent of the draft schedule, starting at a first-payment date that's
either given or defaulted to the end of the first draft's month.
"""
...
def cadence_dates_through_horizon(start: date, horizon: date) -> list[date]:
    """Monthly cadence dates from ``start`` up to and including ``horizon``.

    Open-ended version of ``monthly_payment_dates``: the fee cadence can
    extend past the last creditor-payment date into fee-only dates, so the
    count isn't known up front — generate until a date exceeds the horizon.
    """
    if start > horizon:
        return []
    eom = is_end_of_month(start)
